...
func TestAddProviderMiMoIgnoresForwardAPIKey(t *testing.T) {
	...
	provider, err := AddProvider(context.Background(), model.PlanProviderMiMoPlan, testMiMoCookie, "sk-should-not-be-saved", "MiMo monitor")
	if err != nil {
...
func TestListProvidersMasksPlanProviderSecrets(t *testing.T) {
	...
	provider, err := AddProvider(context.Background(), model.PlanProviderMiMoPlan, testMiMoCookie, "", "MiMo monitor")
	if err != nil {
		t.Fatalf("AddProvider() error = %v", err)
	}
